...
interface SpecificCateringProps {
  catering: CateringWithFeatures | null;
}

const SpecificCatering: React.FC<SpecificCateringProps> = ({
  catering,
}) => {
  return (
    <div className="flex flex-col h-full w-full md:px-80 px-10 py-10">
      <div className="flex justify-between mb-3">
        <div>
          ...
          <div className="flex items-center gap-x-2">
            <p className="font-bold text-2xl">{catering?.name}</p>
      ...
      <Carousel className="w-full mt-5">
        ...
        <CarouselContent className="-ml-1">
          {catering?.inclusions.map((inclusion) => (
            <CarouselItem
              key={inclusion.id}
      ...
      <div className="grid md:grid-cols-4 grid-cols-1 mt-5 gap-4">
        ...
        <div className="bg-zinc-100 h-full mt-[45px] w-full dark:bg-zinc-900 rounded-md p-5">
          <p className="font-semibold text-lg">Catering Higlights</p>
          <p className="text-sm">Status: {catering?.status}</p>
          <ul>
            {catering?.features.map((feature) => (
...
export default SpecificCatering;
