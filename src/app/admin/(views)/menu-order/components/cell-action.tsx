...
import { MenuOrderColumn } from "./column";
...
export const CellAction: React.FC<CellActionProps> = ({ data }) => {
  ...
  const onDelete = async () => {
    setIsLoading(true);
    deleteOrderMenu(data.id)
      .then((data) => {
        if (data.success) {
          toast.success(data.success);
          setOpen(false);
          router.push("/admin/menu-order");
        } else {
  ...
  const onApprove = async () => {
    setIsLoading(true);
    approveOrderMenu(data.id, data.quantity)
      .then((data) => {
        if (data.success) {
          toast.success(data.success);
          setApproveOpen(false);
          window.location.assign("/admin/menu-order");
        } else {
          toast.error(data.error);
        }
      })
